...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse bg-[var(--slot4-line-faint,rgba(225,18,53,0.08))]', className)} />
}
...
export function PageLoadingState({ label = 'Loading page', className }: LoadingStateProps) {
  return (
    <div className={cn('mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8', className)} aria-live="polite" aria-busy="true">
      <p className="deco-label deco-label-sm text-[var(--slot4-muted-text)]">{label}</p>
      <PulseBlock className="mt-5 h-12 w-3/4 max-w-3xl" />
      <PulseBlock className="mt-4 h-5 w-2/3 max-w-2xl" />
      <div className="mt-8 grid gap-px bg-[var(--slot4-line-faint)] md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-5">
...
export function CardGridLoadingState({ count = 6, className }: LoadingStateProps & { count?: number }) {
  return (
    <div className={cn('grid gap-px bg-[var(--slot4-line-faint)] sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="border border-[var(--slot4-line)] bg-[var(--slot4-surface-bg)] p-4">
